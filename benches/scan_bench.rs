...
/// Check if a benchmark ID matches the CLI filter (Criterion passes it as args[1]).
/// Returns true if no filter is set or if the ID contains the filter as a substring.
